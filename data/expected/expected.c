...
__attribute__((export_name("power-of"))) int32_t
__wasm_export_power_power_of(int32_t arg, int32_t arg0) {
...
void power_power_of(int32_t base, int32_t exp,
                    power_expected_s32_string_t *ret0) {
  if (exp < 0) {
    auto res = (power_string_t *)malloc(sizeof(power_string_t));
    res = "error: negative exp";
    ret0->tag = 1;
    ret0->err->ptr = res;
    ret0->err->len = res.size();
    free(res);
  }
}
