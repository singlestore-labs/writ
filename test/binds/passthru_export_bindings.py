...
class Passthru(Protocol):
    @abstractmethod
    def passthru(self, input: str) -> str:
        raise NotImplementedError
    @abstractmethod
    def passthrutwo(self, input1: str, input2: str) -> List[PassOut]:
        raise NotImplementedError
    @abstractmethod
...
def add_passthru_to_linker(linker: wasmtime.Linker, store: wasmtime.Store, host: Passthru) -> None:
    ty = wasmtime.FuncType([wasmtime.ValType.i32(), wasmtime.ValType.i32(), wasmtime.ValType.i32()], [])
    def passthru(caller: wasmtime.Caller, arg0: int, arg1: int, arg2: int) -> None:
        m = caller["memory"]
        ...
        ptr = arg0
        len0 = arg1
        ret = host.passthru(_decode_utf8(memory, caller, ptr, len0))
    ...
    linker.define('passthru', 'passthru', wasmtime.Func(store, ty, passthru, access_caller = True))
    ...
    def passthrutwo(caller: wasmtime.Caller, arg0: int, arg1: int, arg2: int, arg3: int, arg4: int) -> None:
        m = caller["memory"]
        assert(isinstance(m, wasmtime.Memory))
        memory = cast(wasmtime.Memory, m)
        realloc = caller["canonical_abi_realloc"]
        ...
        ptr1 = arg2
        len2 = arg3
        ret = host.passthrutwo(_decode_utf8(memory, caller, ptr, len0), _decode_utf8(memory, caller, ptr1, len2))
        vec = ret
        len6 = len(vec)
    ...
    linker.define('passthru', 'passthrutwo', wasmtime.Func(store, ty, passthrutwo, access_caller = True))
    ...
    linker.define('passthru', 'wit-source-get', wasmtime.Func(store, ty, wit_source_get, access_caller = True))
    ty = wasmtime.FuncType([], [])
    def wit_source_print(caller: wasmtime.Caller) -> None:
        host.wit_source_print()
    linker.define('passthru', 'wit-source-print', wasmtime.Func(store, ty, wit_source_print, access_caller = True))
